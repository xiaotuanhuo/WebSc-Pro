    ...
    TableTree.prototype = {
        ...
        ,doInitTree:function(arr,field,tr,lvl){
            ...
            _layui.each(arr,function (key,item) {
                ...
                if(item.children){
                    ...
                    that.doInitTree(item.children,field,nextTr,lvl+1)
                }
            });
        },
            ...
            for(var i=0;i<arr.length;i++){
                ...
                if(uniqueTreeid+"" === item[treeid]+""){
                    ...
                    if (item.children && item.children.length>0) {
                        var data = this.getDataByTreeId(item.children, uniqueTreeid);
                        if(data) return  data;
                    }
                }
            }
                ...
                if(isAdd){//新增
                    ...
                    var thisObj = {value:null,data:data,field:field,add:function (newTree) {
                            ...
                            while (count<newTree.length){
                                children.push(newTree[count]);count++;
                            }
                ...
                var td = $(this).parents('td')
                    ...
                    ,thisObj = {value:null,data:data,field:field,async:function (asyncTree) {
                        ...
                        while (count<asyncTree.length){
                            data.children.push(asyncTree[count]);count++;
                        }
                        //图标改回来
                        that.openTreeNode(tr);
                    },event:'async'};
        ...
        ,asyncAddTree:function (obj,data,tr,lvl) {
            ...
            obj.forEach(function (e) {
                ...
                if(e.children && e.children.length>0){
                    that.asyncAddTree(e.children,e,tr.next(),lvl+1);
                }
            });
        ...
        ,clearChildCacheData:function (list,uniqueTreeid) {
            ...
            for(var i=0;i<list.length;i++){
                ...
                if((uniqueTreeid+"") === (item[treeid]+"")) {
                    ...
                    if(item.children && item.children.length > 0){
                        this.clearChildCacheData(item.children,uniqueTreeid)
                    }
                }
            }
        }
        ...
        ,refreshTableBody:function (data,tr,lvl,isFirst) {
            ...
            _layui.each(data,function (index,item) {
                ...
                if(item.children){
                    that.refreshTableBody(item.children,newTr,lvl+1,false);
                }
            });
        }
        /**
            ...
            treeData.forEach(function (item) {
                if(treepid in item){
                    if(item[treepid]+"" === treeid+""){
                        ...
                        if(item.children){
                            that.findChildTreeNodes(item.children,treeid,childArr);
                        }
                    }
                }else {
                    if(item.children){
